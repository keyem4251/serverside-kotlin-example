...
class BookControllerTest {
    ...
    @Test
    fun `get List is success`() {
        val bookId = 100L
        val book = Book(bookId, "Kotlin入門", "Kotlin", LocalDate.now())
        val bookList = listOf(BookWithRental(book, null))

        whenever(bookService.getList()).thenReturn(bookList)

        val expectedResponse = GetBookListResponse(listOf(BookInfo(bookId, "Kotlin入門", "Kotlin", false)))
